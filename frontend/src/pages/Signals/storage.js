...
// but with a completely separate storage key (``tcg.signals.v1``) so the
...
// an array of blocks. A block is ``{conditions: Condition[]}``. Condition
// shape is dictated by the backend contract and stored verbatim — the
// storage layer does NOT validate individual condition variants so that
// future new operators don't require a schema bump.
...
// Sanitise a single loaded signal object. Missing direction arrays get
// restored to []; non-array or non-object blocks are dropped; conditions
// are stored verbatim (shape is validated by the backend).
    ...
    rules[dir] = blocks
      ...
      .map((b) => {
        const conditions = Array.isArray(b.conditions)
          ? b.conditions.filter((c) => c && typeof c === 'object' && typeof c.op === 'string')
          : [];
        return { conditions };
      });
  }
