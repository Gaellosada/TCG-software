...
2. Assumptions
3. Data Summary — table + raw-input price-history chart (close + optional benchmark + conditional volume sub-row, rendered immediately beneath the summary table; for options strategies plot the UNDERLYING spot/futures only, never the option chain)
4. Backtest Setup
...
The §3 raw-input chart is generated by `lib.plotting.plot_price_history(bars, benchmark=...)` from the cached `data/<id>.npz` (see `pipeline/02-data.md` for the npz schema) — never re-fetched from MongoDB. The volume sub-row is conditional: skipped automatically when the volume column is missing or all-zero (typical for INDEX series whose providers publish synthetic zero volume). The chart includes a linear/log y-toggle (top-right) on the close-price axis only — volume bars stay linear because log-scaled bars are meaningless. When a benchmark is overlaid and its scale differs from the close by >2x, it's drawn on a secondary y-axis with a `— right axis` legend marker plus a subtitle annotation explaining the rescaling. The canonical generator is `snippets/plot_price_history.py`. For multi-instrument strategies, populate the snippet's `PANELS` list (one `(id, npz_path)` per series) and the snippet writes one `price_history_<id>.json` per panel.
