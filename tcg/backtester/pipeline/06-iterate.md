...
A. The user requests a variant ("what if I widen the SMA window", "try fees 1bp", "extend to 2010").
...
The gate is computed in P4 (see `pipeline/04-analyze.md` → Combiner) and is the single source of truth. P6 never re-derives it from the raw diagnostic values.

## Variant scope decision

Use the table in `pipeline/00-pipeline.md`. Examples:

- "Widen SMA window from 20 to 50" → edit `compute_signal` in `strategy.py`; re-run P3, P4, P5.
- "Add VIX as a regime filter" → edit signal logic; if VIX not in `data/`, add to `META.universe` and re-run P2 first.
- "Use 1% fees" → edit `META["execution"]["fees_bps"]` in `strategy.py`; re-run P3, P4, P5.
- "Extend to 2010" → edit `META["dates"]["start"]`; re-run P2, P3, P4, P5.

All edits happen in `strategy.py` — that is the single source of truth. Never edit a separate YAML file.
...
Run only when `results/diagnostics.json.should_suggest == true`. Pick ONE fired diagnostic by the priority order below (top first), and frame the suggestion as a question. Never run the variant without user confirmation.

| Priority | Diagnostic              | Suggest                                                |
|----------|-------------------------|--------------------------------------------------------|
| 1        | max_drawdown_exceeds    | "Drawdown reached <pct>%. Try a vol-target overlay or a max-DD kill at <X>%?" |
| 2        | regime_concentration    | "MDD concentrated in <period>. Try a regime filter (e.g. VIX > 20)?" |
...
A `high`-severity diagnostic always wins over a `medium`-severity one within the same priority level. Never auto-suggest more than one variant per session.
